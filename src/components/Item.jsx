...
export function Item() {
	const { longTerms, today, deleteItems,  addDoing, updateItems ,removeItem , reorderItems , period} = useList();
	...
	function handleTrackId(id) {
		...
		const itemToEdit = longTerms?.items?.find(item => item.id === id);
	...
	return (
		<>
			<Reorder.Group axis="y" values={longTerms.items} onReorder={(ordine) => reorderItems(ordine)} className="flex items-center justify-between h-full w-full flex-col gap-3.5 text-neutral-50 font-medium">
				<p className="text-xl">Attività da svolgere</p>
				<div className="flex flex-col h-full items-center phone:w-full overflow-y-scroll" >
				{period === "Lungo termine" ? longTerms.items.map((item) => (
				...
				<p className="text-xl">
				<span className="mr-1">Devi svolgere {longTerms.items.length} attività!</span>
				</p>
			</Reorder.Group>
		</>
	);
